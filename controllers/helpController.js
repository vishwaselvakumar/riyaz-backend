...
const Help = require('../modals/helpModels'); // Import your MongoDB model
...
exports.sendHelpMessage = async (req, res) => {
    ...
    try {
        ...
        const newHelp = new Help({ firstname, email, phone, message,companyname });
        await newHelp.save();

        res.status(200).json({ message: 'Help Message sent successfully' });
    } catch (error) {
...
exports.getHelpMessages = async (req, res) => {
    try {
      const messages = await Help.find(); // Fetch all contact messages
